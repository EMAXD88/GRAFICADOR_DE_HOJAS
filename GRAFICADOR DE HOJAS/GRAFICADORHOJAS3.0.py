...
def update_visualization():
    ...
    if not z_min or not z_max:
        messagebox.showerror("Error", "Ingresa valores de rango de z antes de actualizar la visualización.")
        return
    ...
    if len(z) == 0:
        messagebox.showerror("Error", "No se encontraron puntos para graficar con los valores de z especificados.")
        return
    ...
    ax.set_ylabel('Eje Y')
    ax.set_zlabel('Eje Z')
    ax.set_title('MODELO DE HOJA 3D')
    plt.show()
...
def apply_z_range():
    ...
    try:
        ...
        messagebox.showerror("Error", "Ingresa valores numéricos válidos para el rango de z")
