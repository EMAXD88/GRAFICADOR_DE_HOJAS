...
# Función para calcular el área de superficie
def calculate_surface_area(c3d):
    if c3d is None or len(c3d) < 4:
        return 0.0
...
def calculate_volume(c3d):
    if c3d is None or len(c3d) < 2:
        return 0.0

    volume = 0.0
    for i in range(1, len(c3d)):
        x0, y0, z0 = c3d[i - 1]
        x1, y1, z1 = c3d[i]
        volume += (x0 * y1 - x1 * y0) * (z0 + z1) / 2.0

    return abs(volume)

# Función para actualizar la visualización y calcular el área de superficie y el volumen
def update_visualization():
    ...
    for coord_x, coord_y in zip(x, y):
        ...
        if z_min <= z1 <= z_max:
            z.append(z1)
            x1.append((coord_x - cx) * z1 / fx)
            y1.append((coord_y - cy) * z1 / fy)
    ...
    tetra_faces = c3d[Delaunay(c3d[:, 0:2]).simplices]
    ...
    surface_area = calculate_surface_area(c3d)
    surface_area_label.config(text=f"Área de superficie de la figura: {surface_area:.2f} unidades cuadradas")

    # Calcular y mostrar el volumen
    volume = calculate_volume(c3d)
    volume_label.config(text=f"Volumen del objeto: {volume:.2f} unidades cúbicas")
...
# Crear etiquetas para mostrar el área de superficie y el volumen
surface_area_label = tk.Label(root, text="", font=("Helvetica", 14))
surface_area_label.pack(side=tk.TOP, pady=10)

volume_label = tk.Label(root, text="", font=("Helvetica", 14))
volume_label.pack(side=tk.TOP, pady=10)
